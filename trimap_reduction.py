...
X_trimap = embedding.fit_transform(X_norm)
...
with open(RESULTS_DIR / "trimap_time.txt", "w") as f:
    f.write(f"[{datetime.now().isoformat()}] TRIMAP\n")
...
plt.scatter(X_trimap[:, 0], X_trimap[:, 1], alpha=0.05, c='gray', s=0.2, label='AFDB')
plt.legend(markerscale=10)
plt.title("TRIMAP 2D embedding")
...
print(f"Zapisano TRIMAP embedding i obrazy w: {RESULTS_DIR.resolve()}")
